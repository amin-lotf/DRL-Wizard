...
class Runner(object):
    ...
    def restore(self, is_best=False):
        ...
        q1_net_state_dict = torch.load(str(path / f"{prefix_str}q1_net.pt"), map_location=self.device)
        self.policy.q_net1.load_state_dict(q1_net_state_dict)
        q2_net_state_dict = torch.load(str(path / f"{prefix_str}q2_net.pt"), map_location=self.device)
